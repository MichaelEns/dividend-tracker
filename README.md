...
- **Two installable apps**, not one: dividends and bank balances have separate
  manifests, identities and icons, so both can sit on a home screen and open
  independently. See [Two apps, not two tabs](#two-apps-not-two-tabs).
...
tools/               one-off generators (home screen icons)
...
`docs/balances.html` is a second page — and a **second installable app**. It
...
### Two apps, not two tabs

Each page has its own manifest, its own identity and its own icon, so both can
sit on a home screen at once and open independently. There are no links between
them in either header; each footer mentions the other, which is the only way to
find the second one before it is installed.

| | Dividends | Balances |
| --- | --- | --- |
| Manifest | `manifest.webmanifest` | `balances.webmanifest` |
| `id` | `/dividend-tracker/index.html` | `/dividend-tracker/balances.html` |
| Home screen label | Dividends | Balances |
| Icon | navy, rising chart | blue, bank |

Three things about this are easy to get wrong and invisible when you do:

- **The dividend app's `id` has to keep resolving where it always did.** With
  no explicit `id`, a browser derives one from `start_url`. Adding an `id` that
  landed anywhere else would orphan the copy already on your home screen: it
  would stop updating and a second icon would appear beside it.
- **iOS ignores `start_url` and will not accept an SVG touch icon.** Add to Home
  Screen installs whatever page you are looking at, and reads
  `apple-touch-icon`, `apple-mobile-web-app-title` and
  `apple-mobile-web-app-capable` from that page. Without a **PNG** touch icon
  iOS screenshots the page and uses that as the icon. Hence `icon-180.png` and
  `icon-balances-180.png`, generated by `tools/make_icons.py`, square and
  unrounded because iOS applies its own mask.
- **The Balances scope is deliberately narrow** (`./balances.html`). The
  dividend app's scope is the whole directory, so a wider balances scope would
  claim the dividend page too and its links would open in the wrong app.

The service worker had to change for any of this to work. It mapped **every**
navigation onto `./index.html`, which broke both directions once there were two
apps: opening Balances offline served the dividend page, and opening it online
cached the balances HTML *under the index key*, so the dividend app then opened
to Balances the next time it was offline. That second half outlived the
navigation that caused it, which is why it is worth a test rather than a
comment.
...
`tests/pwa.cjs` asks a browser what the unit tests can only read off disk: that
it parses each manifest, that every icon really loads, and that each app opens
to *itself* offline.
...
The offline half took three attempts to become real, which is worth recording.
CDP's `Network.emulateNetworkConditions` does **not** apply to a service
worker's own `fetch`, so the worker reached the network anyway and the check
passed with the bug deliberately reintroduced. Stopping the server was the fix
— but `server.close()` only refuses *new* connections, and the browser held a
keep-alive socket, so it still passed. Then the probe confirming the network
was down turned out to be served from cache by the worker under test.

It now closes every socket and probes a path that never existed, so nothing can
answer it from cache. The check reporting whether the network is genuinely down
is part of the test output for that reason: without it, the assertions below it
were decorative, and they silently were.
